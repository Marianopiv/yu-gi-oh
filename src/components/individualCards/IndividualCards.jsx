...
const IndividualCards = ({ item, chosen, clear }) => {
  return (
    ...
    <div className="flex flex-col gap-1 items-center justify-center text-center p-3 rounded-md m-2 animate__animated animate__fadeIn">
      <h1>{chosen ? "" : item.name}</h1>
      <img
        className={`${chosen?"pt-10 sm:pt-0 w-4/4 h-2/4":"w-56 h-56"} animate__animated animate__fadeIn`}
        src={
          chosen
      ...
      />
      {chosen ? (
        <p className="w-3/4 mt-10 font-semibold text-sm sm:text-lg border-black border rounded-md p-3 animate__animated animate__fadeIn">{chosen.desc}</p>
      ) : (
